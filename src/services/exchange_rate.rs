...
impl ExchangeRateService {
    pub fn new(api_key: &str) -> Self {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .expect(
                "TLS backend should be initialized, or the resolver should load the system configuration.",
            );
    ...
    async fn fetch_latest(&self, base: &str) -> Result<ExchangeRateSnapshot, ExchangeRateError> {
        ...
        let conversion_rates = parsed.conversion_rates.ok_or_else(|| {
            ExchangeRateError::Api("missing conversion_rates in success body".to_string())
        })?;
        let time_next_u64 = parsed
            .time_next_update_unix
            .ok_or_else(|| ExchangeRateError::Api("missing time_next_update_unix".to_string()))?;
        let time_next_i64 = i64::try_from(time_next_u64).map_err(|_| {
            ExchangeRateError::Api("time_next_update_unix is too large".to_string())
        })?;
...
#[cfg(test)]
impl ExchangeRateService {
    pub fn new_test(base_url: String) -> Self {
        let mut service = Self::new("test-key");
        service.base_url = base_url;
        service
    }

    pub fn with_cached_snapshot(snapshot: ExchangeRateSnapshot) -> Self {
        let service = Self::new("test-key");
        *service.cache.write().expect("cache write") = Some(snapshot);
        service
    }
}
